...
import type { Function, Seat } from "@/types"
import { toast } from "sonner"

export default function SeleccionAsientos({
  params,
}: {
  params: { id: string; horario: string }
}) {
  ...
  // Extraemos la información de la sala y el horario
  const [salaId, horario] = params.horario.split("-")
  ...
  useEffect(() => {
    const fetchFunction = async () => {
      try {
        setLoading(true)
        const data = await functionsApi.getOne(functionId)
        setFuncion(data)
      } catch (err) {
          ...
          >
            ...
            >
              <div className="min-w-[600px]">
                <SalaCine
                  filas={funcion.cinemaRoom.seats?.map(seat => seat.row) ?? []}
                  asientos={funcion.cinemaRoom.seats?.map(seat => ({
                    id: seat.id.toString(),
                    row: seat.row,
                    number: seat.number,
                    status: seat.status
